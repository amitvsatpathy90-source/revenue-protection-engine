 ...
 * connecting unauthenticated (ADR-20). The relay's transactional producer additionally needs
 * a TransactionalId ACL (ADR-20 §matrix) on top of this transport auth.
 *
 * <p>PLAINTEXT (explicit lab choice) returns only the protocol key — a behavioural no-op versus
 * today's implicit default, so the Testcontainers suite is unaffected.
 */
