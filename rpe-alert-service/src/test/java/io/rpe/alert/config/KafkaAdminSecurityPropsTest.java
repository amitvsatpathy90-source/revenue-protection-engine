 ...
 * {@code rpe.dlt.depth} gauge freezes at its boot value 0 — false-healthy, silently disarming
 * the ADR-23 RpeDlt* alerts. The Testcontainers suites run PLAINTEXT, where the fold is a no-op,
 * so this test is the only CI guard on the SASL path.
 */
class KafkaAdminSecurityPropsTest {

    @Test
    void adminClientConfigCarriesSaslTransportProps() {
        AlertKafkaConfig config = new AlertKafkaConfig(
                "SASL_SSL", "SCRAM-SHA-512", "rpe-alert", "secret", "", "");
        ...
        assertThat(String.valueOf(props.get(SaslConfigs.SASL_JAAS_CONFIG)))
                .contains("username=\"rpe-alert\"");
    }

    @Test
    void plaintextLabDefaultStaysNoOp() {
        AlertKafkaConfig config = new AlertKafkaConfig("PLAINTEXT", "SCRAM-SHA-512", "", "", "", "");
        KafkaAdmin kafkaAdmin = new KafkaAdmin(Map.of(
