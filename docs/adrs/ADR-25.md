...
tracing adds no endpoint), the security discipline (PII rule — no `account_id`/payload in spans),
...
**5. PII discipline (the security discipline, unchanged).** Span names are operation names (`rpe.outbox.relay`,
...
- **R4 — Span cardinality / PII.** Span attributes must never carry `account_id`/payload (the security discipline);
