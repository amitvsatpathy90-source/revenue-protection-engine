...
- Embedding-client retry wiring confirmed via bytecode (2026-09-12) — no duplicate-retry blast radius on the embedding boundary, matching the already-fixed chat path.
- `gen_ai` embedding spans confirmed auto-instrumented (2026-09-12, docs) — `OpenAiEmbeddingModel` takes `ObservationRegistry`; Spring AI ships a dedicated `spring-ai-autoconfigure-model-embedding-observation` module consumed by the OpenAI autoconfig. ADR-25's tracing-coverage claim holds for the explicit `embed()` call path with no manual `Observation` wrap needed.
- `EvidenceValidator`/`ragSources[]` extension closed as not-applicable — confirmed against live `TriageAgent.java`: RAG context reaches only the prompt's fenced `<rag_context>` block, never `evidenceValidator.validate()` or `TriageVerdict`. RAG stays narrative-only by design; zero evidence-path changes needed.
...
- **Embedding-client retry wiring — RESOLVED (2026-09-12, bytecode-confirmed):** `javap -p -c` against `OpenAiEmbeddingAutoConfiguration` (`spring-ai-autoconfigure-model-openai:2.0.0`) shows a call to `OpenAiCommonProperties.getMaxRetries()` inside the `openAiEmbeddingModel` bean method — the identical shared field ADR-15/RPE-10 already fixed to `0` for chat. Embedding inherits `max-retries: 0`; no separate override required. Closes testing bar #7's blocking condition.
...
| Item | Note |
|---|---|
| Corpus staleness metric | Built (2026-09-12): `rag.corpus.last_ingested_at` gauge in `JdbcTriageVectorRepository`, cached-at-startup (`AtomicLong` set once from `MAX(created_at)`) — matches one-time hand-authored ingestion; will NOT reflect a live reseed without restart. Alerting rule not yet configured. |
...
7. **Embedding-retry double-fire test** — unblocked (2026-09-12, see Negative section): embedding confirmed to inherit chat's `max-retries: 0`. Assert exactly one retry sequence fires on a 429 — R4j's, not a duplicate SDK-level retry.
...
| 1.3.0 | 2026-09-12 | @amit | Amendment — PROPOSED. Closes 3 Residuals via verification pass: embedding-retry wiring confirmed (bytecode), `gen_ai` embedding spans confirmed (docs), `EvidenceValidator`/`ragSources[]` closed as not-applicable (RAG narrative-only by design). `rag.corpus.last_ingested_at` staleness metric implemented (cached-at-startup). |