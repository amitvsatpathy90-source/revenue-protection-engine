...
/**
 * Pins the triaged_alerts alert_id PRIMARY KEY against schema drift.
 *
 * Raw inserts deliberately bypass ON CONFLICT so a weakened constraint cannot be masked
 * by application-level duplicate handling. Only V1 is migrated; the RAG migrations are
 * unrelated to this persistence invariant.
 */
@Testcontainers(disabledWithoutDocker = true)
class TriageInboxConstraintTest {
    ...
    void duplicateAlertId_rejectedByPostgresConstraint() {
        ...
        jdbcTemplate.update("""
                INSERT INTO triaged_alerts (alert_id, account_id, rule_name)
                VALUES (?, ?, ?)
                """, alertId, "constraint-test", "velocity");
